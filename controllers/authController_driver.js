...
module.exports.cancelRideRequest = async function(req, res) {
  try {
      ...
      if (rideRequest.status === 'cancelled') {
          return res.status(400).json({ message: "This ride request has already been canceled." });
      }

      rideRequest.driverId = null; // Remove the driver association
      rideRequest.status = 'pending'; // Make it available for other drivers
...
module.exports.getScheduledRideRequests = async function(req, res) {
    try {
        ...
        const scheduledRides = await ScheduledRide.find({
          $or: [
            { carType: driver.carType, driverId: null }, // Scheduled rides matching the driver's car type
            { carType: 'Any', driverId: null } // Scheduled rides of type "Any"
        ]
        }).populate('riderId');
...
module.exports.confirmScheduledRide = async function(req, res) {
    ...
    try {
        ...
        // Update the carType to the driver's car type if it is "Any"
        if (scheduledRide.carType === 'Any' ) {
          scheduledRide.carType = req.session.user.carType; // Set to the driver's car type
      }
...
module.exports.cancelScheduledRide = async function(req, res) {
    ...
    try {
        ...
        const mailOptions = {
            ...
            text: `Your scheduled ride has been cancelled by the driver.`
        };

        await transporter.sendMail(mailOptions);
        res.json({ message: "Scheduled ride cancelled and email sent to the rider." });
    } catch (error) {
