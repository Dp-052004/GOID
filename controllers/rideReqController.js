...
const scheduleRide = async (req, res) => {
  try {
      ...
      const { carType, scheduleTime, rideType } = req.body;
      const riderId = req.session.user._id;

      // Determine the pickup address based on the ride type
      const pickupAddress = rideType === 'Home to Office' ? 'Home Address' : 'Office Address'; // Set pickup address based on ride type
      ...
      const newScheduledRide = await ScheduledRide.create({
        ...
        pickup: {
            type: "Point",
            coordinates: [pickupAddress.longitude, pickupAddress.latitude], // Use the pickup address from the form
        },
          driverId: null,
      });
      ...
      // If carType is "Any", fetch all available drivers
      if (carType === "Any") {
        availableDrivers = await Driver.find({ isAvailable: true }); // Fetch all available drivers
    } else {
        // Otherwise, fetch drivers matching the requested car type
        availableDrivers = await Driver.find({
            isAvailable: true,
            carType: carType,
        });
...
const cancelScheduledRide = async (req, res) => {
  const { rideId } = req.params; // Get the ride ID from the URL
  const  riderEmail  = req.session.user.eEmail; // Get the rider's email from the request body

  try {
      const scheduledRide = await ScheduledRide.findById(rideId);
      if (!scheduledRide) {
          return res.status(404).json({ message: "Scheduled ride not found." });
      }

      // Update the scheduled ride to cancel it
      scheduledRide.driverId = null; // Remove the driver association
      scheduledRide.status = 'cancelled'; // Make it available for other drivers
      await scheduledRide.save();

      // Check if riderEmail is defined
      if (!riderEmail) {
          return res.status(400).json({ message: "Rider email is not defined." });
      }

      // Send cancellation email to the rider
      const transporter = nodemailer.createTransport({
          host: 'smtp.gmail.com',
          port: 465,
          secure: true,
      ...
      const mailOptions = {
          ...
          text: `Your scheduled ride has been cancelled successfully.`,
      };

      await transporter.sendMail(mailOptions);
      console.log(`Cancellation email sent to: ${riderEmail}`);

      res.json({ success: true, message: "Scheduled ride canceled and email sent to the rider." });
  } catch (error) {
